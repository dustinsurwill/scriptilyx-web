...
import { findStartNodes } from '../../../lib/graph'
import { buildVariableRegistry } from '../../../lib/variableRegistry'
...
export function generateScript(
  ...
  // RunEverySeconds/WaitSeconds only work if Main() actually gets invoked on
  // a recurring basis — that's what Runtime.UpdateFrequency controls, and
  // the game's own Program() template calls out setting it there as "what
  // lets your script run itself without a timer block". If the graph uses
  // one of those timing nodes but never sets an update frequency itself, the
  // PB just sits idle until something else triggers it, which reads as "the
  // timer doesn't work". So default one in rather than ship a script that's
  // silently inert.
  const TIMING_ACTION_TYPES = new Set(['RunEverySeconds', 'WaitSeconds'])
  const needsAutoUpdateFrequency =
    reachable.some((n) => TIMING_ACTION_TYPES.has(n.ActionType)) &&
    !reachable.some((n) => n.ActionType === 'SetRuntimeUpdate')
  if (needsAutoUpdateFrequency) {
    warnings.push(
      'Added "Runtime.UpdateFrequency = UpdateFrequency.Update10;" to Program() automatically because the graph ' +
        'uses a "Run Every Seconds"/"Wait Seconds" node but never sets an update frequency itself — otherwise Main() ' +
        'only runs when something else triggers it. Add a "Set Runtime Update Frequency" node if you want control over this.',
    )
  }

  const lines: string[] = []
  if (options.professionalComments) {
    lines.push(`// Generated by WireRig from a ${nodes.length}-node graph (${connections.length} connections).`)
    lines.push(`// Start node: "${start.Title}" (#${start.Number}).`)
    lines.push(`// Steps in this script: ${reachable.length} of ${nodes.length} nodes are reachable from Start.`)
  ...
  if (needsAutoUpdateFrequency) lines.push(`    Runtime.UpdateFrequency = UpdateFrequency.Update10;`)
  lines.push(`}`)
  lines.push('')
  ...
  if (tickBudget) {
    ...
    for (const { key, node, statements } of bodies) {
      ...
      if (options.professionalComments && node.Description) lines.push(`        // ${node.Description}`)
